...
class TestDispatchSystemPrompt:
    ...
    def test_ordinary_prompt_not_dispatch_lean(self):
        from agent.system_prompt import HERMES_AGENT_HELP_GUIDANCE, build_system_prompt_parts
        ...
        parts = build_system_prompt_parts(agent)
        assert HERMES_AGENT_HELP_GUIDANCE in parts["stable"]
...
class TestDispatchStartupReduction:
    def test_dispatch_fixture_smaller_than_default_api_server(self):
        ...
        base_parts = build_system_prompt_parts(base_agent)
        dispatch_parts = build_system_prompt_parts(dispatch_agent)
